...
gastosRouter.get('/', async (req, res) => {
  ...
  const { rows } = await pool.query(
    `SELECT * FROM gastos WHERE 1=1 ${where} ORDER BY fecha DESC`,
    params
  );
  res.json({ data: rows });
});
...
gastosRouter.post('/', async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [Number(b.vehiculo_id), b.fecha, b.categoria || 'otro', Number(b.monto), b.descripcion || null]
  );
...
gastosRouter.put('/:id', async (req, res) => {
  ...
  const { rows } = await pool.query(
    ...
    [
      ...
      b.fecha || undefined,
      b.categoria || undefined,
      b.monto !== undefined ? Number(b.monto) : undefined,
